...
import (
	...
	http_dailer "github.com/mwitkow/go-http-dialer"
...
func NewClient(config *ssh.ClientConfig) transport.Transport {
	return gcommon.TNewClient(&runner{config: config})
}
...
func (r *runner) Command(cmd string, ep *transport.Endpoint, auth transport.AuthMethod) (gcommon.Command, error) {
...
func dial(network, addr string, proxyOpts transport.ProxyOptions, config *ssh.ClientConfig) (*ssh.Client, error) {
	...
	if strings.HasPrefix(proxyOpts.URL, "socks5") {
		...
		if !ok {
			return nil, fmt.Errorf(
				"expected ssh proxy dialer to be of type %s; got %s",
				reflect.TypeOf(ctxDialer),
				reflect.TypeOf(dialer),
			)
		}
		...
		httpTunnel := http_dailer.New(proxyUrl)
		conn, err = httpTunnel.Dial(network, addr)
	} else {
