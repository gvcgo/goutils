...
func (that *GhRemoter) CreateRepo(repoName string) (r []byte) {
...
func (that *GhRemoter) GetRepoInfo(repoName string) (r []byte) {
...
func (that *GhRemoter) GetContents(repoName, remotePath, fileName string) (r []byte) {
...
*/
func (that *GhRemoter) UploadFile(repoName, remotePath, localPath, shaStr string) (r []byte) {
...
func (that *GhRemoter) GetFileInfo(repoName, remotePath, fileName string) (r []byte) {
	// https://api.github.com/repos/{user}/{repo}/contents/{path}/{filename}
	remotePath = strings.TrimLeft(filepath.Join(remotePath, fileName), "/")
	that.fetcher.SetUrl(fmt.Sprintf("%s/repos/%s/%s/contents/%s", GithubAPI, that.UserName, repoName, remotePath))
	that.fetcher.Timeout = 30 * time.Minute
...
func (that *GhRemoter) DeleteFile(repoName, remotePath, fileName, shaStr string) (r []byte) {
	// https://api.github.com/repos/{user}/{repo}/contents/{path}/{filename}
	remotePath = strings.TrimLeft(filepath.Join(remotePath, fileName), "/")
	that.fetcher.SetUrl(fmt.Sprintf("%s/repos/%s/%s/contents/%s", GithubAPI, that.UserName, repoName, remotePath))
	that.fetcher.Timeout = 30 * time.Minute
